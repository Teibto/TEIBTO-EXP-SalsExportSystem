/**
 * @NApiVersion 2.x
 * @NScriptType UserEventScript
 *
 * @summary     userevent script for EXP Custom Button
 * @author      Rakop M. [2024/07/15] <rakop@teibto.com>
 *
 * @version     1.0
 */

var CS_SCRIPT_PATH = null;
var EXP_ENABLE_FEATURES = {};
var CURRENT_RECORD, REC_TYPE, REC_ID, FORM;
var USE_SUB, USER_ROLE_ID, USER_ID, TYPE, REQUEST, PARAMS, EXECUTION_CONTEXT;
var config, format, record, runtime, search, task, error, url, redirect;
define(['N/config', 'N/format', 'N/record', 'N/runtime', 'N/search', 'N/task', 'N/error', 'N/url', 'N/redirect'],

    /**
     * @param {type} _config
     * @param {type} _format
     * @param {type} _record
     * @param {type} _runtime
     * @param {type} _search
     * @param {type} _task
     * @param {type} _error
     * @param {type} _url
     * @param {type} _redirect
     * @return {}
     */
    function(_config, _format, _record, _runtime, _search, _task, _error, _url, _redirect){
        config = _config;
        format = _format;
        record = _record;
        runtime = _runtime;
        search = _search;
        task = _task;
        error = _error;
        url = _url;
        redirect = _redirect;
        USER_ROLE_ID = runtime.getCurrentUser().role;
        USER_ID = runtime.getCurrentUser().id;
        USE_SUB = runtime.isFeatureInEffect('SUBSIDIARIES');
        EXECUTION_CONTEXT = runtime.executionContext;
        TYPE = '';
        PARAMS = {};

        return {
            beforeLoad: beforeLoad,
            // beforeSubmit: beforeSubmit,
            // afterSubmit: afterSubmit
        };
    }
);

// ################################################################################################
// ===== User Event Function
// ################################################################################################
function beforeLoad(context) {
    try {
        TYPE = context.type
        REQUEST = context.request;
        if (!!REQUEST) PARAMS = REQUEST.parameters;

        if(TYPE == 'delete'){ return; }

        CURRENT_RECORD = context.newRecord;
        REC_TYPE = CURRENT_RECORD.type;
        REC_ID = CURRENT_RECORD.id;
        FORM = context.form;

        //################################################################################################
        //========== Display Loading
        //################################################################################################
        // try {
        //     if (TYPE == 'view') {
        //         var loadingImg = getImageLoading();
        //         var loadingHTML = '';
        //             loadingHTML += '<div id="loadingScreen" style="position: fixed; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(127, 140, 141, 0.75); z-index: 1001; cursor:wait; display:none;">';// display:none
        //             loadingHTML += ' <table border="0" width="100%" height="100%"><tr><td align="center" valign="middle">';
        //             loadingHTML += ' <table border="0" width="200px" height="100px" style="border-radius: 5px; background-color: rgba(10,10,10, 0.8); color: rgba(149, 165, 166, 1.0);">';
        //             loadingHTML += ' <tr><td align=center valign="middle" style="padding:0px 20px;font-weight: bold; font-size:13pt;">';
        //             loadingHTML += ' Loading...<br><img width="220" height="25" src="' + loadingImg + '"/>';
        //             loadingHTML += ' </td>';
        //             loadingHTML += ' </tr>';
        //             loadingHTML += ' </table>';
        //             loadingHTML += ' </td></tr></table>';
        //             loadingHTML += '</div>';
        //         var field = FORM.addField({ id: 'custpage_exp_loading', type: 'inlinehtml', label: ' ' });
        //             field.defaultValue = loadingHTML;
        //     }
        // }catch(e){
        //     log.error('beforeLoad | '+TYPE, e);
        // }

        //################################################################################################
        // ===== Load Sales Order Button
        //################################################################################################
        try {
            if(TYPE == 'view' && REC_TYPE == 'salesorder') {
                EXP_ENABLE_FEATURES = getEXPEnableFeatures();
                // log.debug('EXP_ENABLE_FEATURES', EXP_ENABLE_FEATURES);
            }
        } catch(e) {
            log.error('beforeLoad | '+TYPE+' | '+USER_ROLE_ID+' | '+REC_TYPE+' | '+REC_ID, e);
        }

        //################################################################################################
        // ===== Sales Order - Split Button
        //################################################################################################
        try {
            if(TYPE == 'view' && REC_TYPE == 'salesorder') {
                if (!!EXP_ENABLE_FEATURES['custrecord_exps_allowsplitproformainv'] && EXP_ENABLE_FEATURES['custrecord_exps_allowsplitproformainv'] == true) {
                    var ordertype_id = CURRENT_RECORD.getValue('custbody_exp_ordertype');
                    var status_ref = CURRENT_RECORD.getValue('statusRef');
                    var original_profoma = CURRENT_RECORD.getValue('custbody_exp_originalprofoma');
                    if (ordertype_id == 2 && status_ref == 'pendingFulfillment' && !original_profoma) { // 2 = Proforma Invoice

                        var scriptParams = {};
                            scriptParams['step'] = 'SplitProformaInvoice';
                            scriptParams['rec_id'] = REC_ID;
                        var scriptURL = url.resolveScript({
                            scriptId: 'customscript_exp_sl_split_pro_inv',
                            deploymentId: 'customdeploy1',
                            params: scriptParams,
                            returnExternalUrl: false
                        });

                        var script_function = "page_init;";
                            script_function += "function expSplit(){ window.location = '" + scriptURL + "'; }";
                            script_function += "expSplit;";

                        var button = FORM.addButton({
                            id: 'custpage_exp_split',
                            label: 'Split',
                            functionName: script_function
                        });
                    }
                }
            }
        } catch(e) {
            log.error('beforeLoad | '+TYPE+' | '+USER_ROLE_ID+' | '+REC_TYPE+' | '+REC_ID, e);
        }

        //################################################################################################
        // ===== Sales Order - Cancel Button
        //################################################################################################
        try {
            if(TYPE == 'view' && REC_TYPE == 'salesorder') {
                if (!!EXP_ENABLE_FEATURES['custrecord_exps_allowsplitproformainv'] && EXP_ENABLE_FEATURES['custrecord_exps_allowsplitproformainv'] == true) {
                    var ordertype_id = CURRENT_RECORD.getValue('custbody_exp_ordertype');
                    var status_ref = CURRENT_RECORD.getValue('statusRef');
                    var original_profoma = CURRENT_RECORD.getValue('custbody_exp_originalprofoma');
                    if (ordertype_id == 2 && status_ref != 'closed' && !!original_profoma) { // 2 = Proforma Invoice

                        var scriptParams = {};
                            scriptParams['step'] = 'CancelProformaInvoice';
                            scriptParams['rec_id'] = REC_ID;
                        var scriptURL = url.resolveScript({
                            scriptId: 'customscript_exp_sl_split_pro_inv',
                            deploymentId: 'customdeploy1',
                            params: scriptParams,
                            returnExternalUrl: false
                        });

                        var script_function = "page_init;";
                            script_function += "function expCancel(){ window.location = '" + scriptURL + "'; }";
                            script_function += "expCancel;";

                        var button = FORM.addButton({
                            id: 'custpage_exp_cancel',
                            label: 'Cancel Split PI',
                            functionName: script_function
                        });
                    }
                }
            }
        } catch(e) {
            log.error('beforeLoad | '+TYPE+' | '+USER_ROLE_ID+' | '+REC_TYPE+' | '+REC_ID, e);
        }

        //################################################################################################
        // ===== Sales Order - Enter Booking Button
        //################################################################################################
        try {
            if(TYPE == 'view' && REC_TYPE == 'salesorder') {
                var ordertype_id = CURRENT_RECORD.getValue('custbody_exp_ordertype');
                var status_ref = CURRENT_RECORD.getValue('statusRef');
                if (ordertype_id == 3 && status_ref == 'pendingFulfillment') { // 3 = Commercial Invoice
                    if (!isBookingConfirmed(REC_ID)) {
                        var scriptParams = {};
                            scriptParams['step'] = 'EnterBooking';
                            scriptParams['rec_id'] = REC_ID;
                        var scriptURL = url.resolveScript({
                            scriptId: 'customscript_exp_sl_booking',
                            deploymentId: 'customdeploy1',
                            params: scriptParams,
                            returnExternalUrl: false
                        });

                        var script_function = "page_init;";
                            script_function += "function expEnterBooking(){ window.location = '" + scriptURL + "'; }";
                            script_function += "expEnterBooking;";

                        var button = FORM.addButton({
                            id: 'custpage_exp_enter_booking',
                            label: 'Enter Booking',
                            functionName: script_function
                        });
                    }
                }
            }
        } catch(e) {
            log.error('beforeLoad | '+TYPE+' | '+USER_ROLE_ID+' | '+REC_TYPE+' | '+REC_ID, e);
        }

        //################################################################################################
        // ===== Sales Order - Loading Plan Button
        //################################################################################################
        try {
            if(TYPE == 'view' && REC_TYPE == 'salesorder') {
                var ordertype_id = CURRENT_RECORD.getValue('custbody_exp_ordertype');
                var status_ref = CURRENT_RECORD.getValue('statusRef');
                if (ordertype_id == 3 && status_ref == 'pendingFulfillment') { // 3 = Commercial Invoice
                    if (isBookingConfirmed(REC_ID) && !isLoadingPlanConfirmed(REC_ID)) {
                        var scriptParams = {};
                            scriptParams['step'] = 'LoadingPlan';
                            scriptParams['rec_id'] = REC_ID;
                        var scriptURL = url.resolveScript({
                            scriptId: 'customscript_exp_sl_loading_plan',
                            deploymentId: 'customdeploy1',
                            params: scriptParams,
                            returnExternalUrl: false
                        });

                        var script_function = "page_init;";
                            script_function += "function expLoadingPlan(){ window.location = '" + scriptURL + "'; }";
                            script_function += "expLoadingPlan;";

                        var button = FORM.addButton({
                            id: 'custpage_exp_loading_plan',
                            label: 'Loading Plan',
                            functionName: script_function
                        });
                    }
                }
            }
        } catch(e) {
            log.error('beforeLoad | '+TYPE+' | '+USER_ROLE_ID+' | '+REC_TYPE+' | '+REC_ID, e);
        }

        //################################################################################################
        // ===== EXP Booking - Cancel Button
        //################################################################################################
        try {
            if(TYPE == 'view' && REC_TYPE == 'customrecord_exp_booking') {
                var bookingstatus_id = CURRENT_RECORD.getValue('custrecord_bk_bookingstatus');
                if (bookingstatus_id == 1) { // 1 = Confirmed

                    var scriptParams = {};
                        scriptParams['step'] = 'InProgressCancelBooking';
                        scriptParams['rec_id'] = REC_ID;
                    var scriptURL = url.resolveScript({
                        scriptId: 'customscript_exp_sl_booking',
                        deploymentId: 'customdeploy1',
                        params: scriptParams,
                        returnExternalUrl: false
                    });

                    var script_function = "page_init;";
                        script_function += "function expCancelBooking(){ window.location.href = '"+scriptURL+"'; };";
                        script_function += "if(!confirm('Would you like to Cancel?')) return false; ";
                        script_function += "else expCancelBooking();";

                    var button = FORM.addButton({
                        id: 'custpage_exp_cancel_booking',
                        label: 'Cancel',
                        functionName: script_function
                    });
                }
            }
        } catch(e) {
            log.error('beforeLoad | '+TYPE+' | '+USER_ROLE_ID+' | '+REC_TYPE+' | '+REC_ID, e);
        }

        //################################################################################################
        // ===== EXP Loading Plan - Confirm Button
        //################################################################################################
        try {
            if(TYPE == 'view' && REC_TYPE == 'customrecord_exp_loadplan') {
                var lp_status_id = CURRENT_RECORD.getValue('custrecord_lp_status');
                if (lp_status_id != 2) { // 2 = Confirmed

                    var scriptParams = {};
                        scriptParams['step'] = 'InProgressConfirmLoadingPlan';
                        scriptParams['rec_id'] = REC_ID;
                    var scriptURL = url.resolveScript({
                        scriptId: 'customscript_exp_sl_loading_plan',
                        deploymentId: 'customdeploy1',
                        params: scriptParams,
                        returnExternalUrl: false
                    });

                    var script_function = "page_init;";
                        script_function += "function expConfirmLoadingPlan(){ window.location.href = '"+scriptURL+"'; };";
                        script_function += "if(!confirm('Would you like to Confirm?')) return false; ";
                        script_function += "else expConfirmLoadingPlan();";

                    var button = FORM.addButton({
                        id: 'custpage_exp_confirm_loading_plan',
                        label: 'Confirm',
                        functionName: script_function
                    });
                }
            }
        } catch(e) {
            log.error('beforeLoad | '+TYPE+' | '+USER_ROLE_ID+' | '+REC_TYPE+' | '+REC_ID, e);
        }

        //################################################################################################
        // ===== EXP Loading Plan - Cancel Button
        //################################################################################################
        try {
            if(TYPE == 'view' && REC_TYPE == 'customrecord_exp_loadplan') {
                var lp_status_id = CURRENT_RECORD.getValue('custrecord_lp_status');
                if (lp_status_id != 3) { // 3 = Cancelled

                    var scriptParams = {};
                        scriptParams['step'] = 'SubmitCancelLoadingPlan';
                        scriptParams['rec_id'] = REC_ID;
                    var scriptURL = url.resolveScript({
                        scriptId: 'customscript_exp_sl_loading_plan',
                        deploymentId: 'customdeploy1',
                        params: scriptParams,
                        returnExternalUrl: false
                    });

                    var script_function = "page_init;";
                        script_function += "function expCancelLoadingPlan(){ window.location.href = '"+scriptURL+"'; };";
                        script_function += "if(!confirm('Would you like to Cancel?')) return false; ";
                        script_function += "else expCancelLoadingPlan();";

                    var button = FORM.addButton({
                        id: 'custpage_exp_cancel_loading_plan',
                        label: 'Cancel',
                        functionName: script_function
                    });
                }
            }
        } catch(e) {
            log.error('beforeLoad | '+TYPE+' | '+USER_ROLE_ID+' | '+REC_TYPE+' | '+REC_ID, e);
        }

        //################################################################################################
        // ===== EXP Loading Plan - Redirect To Suitelet in Edit Mode
        //################################################################################################
        try {
            if(TYPE == 'edit' && REC_TYPE == 'customrecord_exp_loadplan' && EXECUTION_CONTEXT == 'USERINTERFACE') {
                var redirectParams = {};
                    redirectParams['step'] = 'LoadingPlan';
                    redirectParams['rec_id'] = CURRENT_RECORD.getValue('custrecord_lp_transaction');
                    redirectParams['loading_plan_id'] = REC_ID;

                redirect.toSuitelet({
                    scriptId: 'customscript_exp_sl_loading_plan',
                    deploymentId: 'customdeploy1',
                    parameters : redirectParams
                });
            }
        } catch(e) {
            log.error('beforeLoad | '+TYPE+' | '+USER_ROLE_ID+' | '+REC_TYPE+' | '+REC_ID, e);
        }

    } catch(e) {
        log.error('beforeLoad | '+TYPE+' | '+USER_ROLE_ID+' | '+REC_TYPE+' | '+REC_ID, e);
    }
}

function beforeSubmit(context) {
    try {

        TYPE = context.type
        REQUEST = context.request;
        if (!!REQUEST) PARAMS = REQUEST.parameters;

        if(TYPE == 'delete'){ return; }

        CURRENT_RECORD = context.newRecord;
        REC_TYPE = CURRENT_RECORD.type;
        REC_ID = CURRENT_RECORD.id;

        try {  
            if(TYPE == 'create' || TYPE == 'edit'){ 

            }
        } catch(e) {
            log.error('beforeSubmit | '+TYPE+' | USER_ROLE_ID = '+USER_ROLE_ID+' | USER_ID = '+USER_ID, e);
        }

    } catch(e) {
        log.error('beforeSubmit | '+TYPE+' | USER_ROLE_ID = '+USER_ROLE_ID+' | USER_ID = '+USER_ID, e);
    }
}

function afterSubmit(context) {
    try {

        TYPE = context.type
        REQUEST = context.request;
        if (!!REQUEST) PARAMS = REQUEST.parameters;

        if(TYPE == 'delete'){ return; }

        CURRENT_RECORD = context.newRecord;
        REC_TYPE = CURRENT_RECORD.type;
        REC_ID = CURRENT_RECORD.id;

        try {
            if(TYPE == 'create'){

            }
        } catch(e) {
            log.error('afterSubmit 1 | '+TYPE+' | USER_ROLE_ID = '+USER_ROLE_ID+' | USER_ID = '+USER_ID, e);
        }
        
    } catch(e) {
        log.error('afterSubmit | '+TYPE+' | USER_ROLE_ID = '+USER_ROLE_ID+' | USER_ID = '+USER_ID, e);
    }
}
// ################################################################################################
// ===== User Event Library Function
// ################################################################################################
function getEXPEnableFeatures() {
    var EXPEnableFeatures = {};
    var filterSearch = [];
        filterSearch.push( search.createFilter({ name: 'isinactive', join: null, operator: 'is', values: 'F' }) );
    var columnSearch = [];
        columnSearch.push(search.createColumn({ name: 'internalid', summary: null }));
        columnSearch.push(search.createColumn({ name: 'custrecord_exps_allowsplitproformainv', summary: null }));
        columnSearch.push(search.createColumn({ name: 'custrecord_exps_splitrunning', summary: null }));
        columnSearch.push(search.createColumn({ name: 'custrecord_exps_prefixsplit', summary: null }));
    var ssResult = getSearch('customrecord_exp_enablefeatures', null, columnSearch, filterSearch);
    if (ssResult.length > 0) {
        var columns = ssResult[0].columns;
        for (var i = 0; i < columns.length; i++) {
            var id = columns[i].name;
            EXPEnableFeatures[id] = ssResult[0].getValue(columns[i]);
        }
    }
    return EXPEnableFeatures;
}

function isBookingConfirmed(rec_id) {
    var isConfirmed = false;
    var filterSearch = [];
        filterSearch.push( search.createFilter({ name: 'custrecord_bk_transaction', join: null, operator: 'is', values: rec_id }) );
        filterSearch.push( search.createFilter({ name: 'custrecord_bk_bookingstatus', join: null, operator: 'is', values: 1 }) ); // Confirmed
    var columnSearch = [];
        columnSearch.push(search.createColumn({ name: 'internalid', summary: null }));
    var ssResult = getSearch('customrecord_exp_booking', null, columnSearch, filterSearch);
    if (ssResult.length > 0) {
        isConfirmed = true;
    }
    return isConfirmed;
}

function isLoadingPlanConfirmed(rec_id) {
    var isConfirmed = false;
    var filterSearch = [];
        filterSearch.push( search.createFilter({ name: 'custrecord_lp_transaction', join: null, operator: 'is', values: rec_id }) );
        filterSearch.push( search.createFilter({ name: 'custrecord_lp_status', join: null, operator: 'anyof', values: [1,2] }) ); // 1 = Pending Confirm, 2 = Confirmed
    var columnSearch = [];
        columnSearch.push(search.createColumn({ name: 'internalid', summary: null }));
    var ssResult = getSearch('customrecord_exp_loadplan', null, columnSearch, filterSearch);
    if (ssResult.length > 0) {
        isConfirmed = true;
    }
    return isConfirmed;
}

// ################################################################################################
// ===== Library Function
// ################################################################################################
function getSearch(recordType, searchId, columns, filters){
    var searchResults = [],
    searchOptions = {},
    searchObj = {};

    if(!!searchId){
        searchObj = search.load( {id: searchId} );

        //Copy the filters from objSearch into default Filters, Columns
        searchOptions.columns = searchObj.columns;
        searchOptions.filters = searchObj.filters

        //Push the new filter, column into default Filters, Columns
        for(var i in columns){
            searchOptions.columns.push(columns[i]);
        }

        for(var i in filters){
            searchOptions.filters.push(filters[i]);
        }

        //Copy the modified default Filters, Columns back into searchObj
        searchObj.columns = searchOptions.columns;
        searchObj.filters = searchOptions.filters;
    }else{
        if(!!recordType){ searchOptions.type = recordType; }
        if(!!columns){ searchOptions.columns = columns; }
        if(!!filters){ searchOptions.filters = filters; }

        searchObj = search.create( searchOptions );
    }

    var myPagedData = {};
    var myPage = {};
    var i = 0;

    myPagedData = searchObj.runPaged({pageSize: 1000});
    myPagedData.pageRanges.forEach(function(pageRange){
        myPage = myPagedData.fetch({index: pageRange.index});
        myPage.data.forEach(function(result){
            searchResults.push(result);
        });
    });

    return searchResults;
}

function getImageLoading() {
    return 'data:image/gif;base64,';
}
